<template>
    <view class="feitui-select" @click.stop="trigger" :style="[feituiSelectSize]">
		...
		<view style="padding: 10px 0 0 15px;color: white;">{{value}}
        ...
        <view class="feitui-select-suffix" :style="{border: '1px solid rgba（0,0,0,0)'}" :class="[showSuffix]">
            <view class="feitui-select-down-tag">^</view>
        </view>
        <view class="feitui-select-options" v-if="showOptions"
            ...
            <view class="feitui-select-options-item" v-for="item in options" :key="item.value"
...
<script>
    ...
    const OPTIONS_ITEM_HEIGHT = 33 // 修改务必也修改feitui-select-options-item的css部分
