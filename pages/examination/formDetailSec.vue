<template>
	<view>
		...
		<uni-popup ref="popupSec" type="center" style="background-color: white;">
...
<script>
	...
	export default {
		...
		methods: {
			...
			fdClick:function(e){
				if(this.fdIndex!==e.detail.value){
					 this.fdIndex=e.detail.value
					 this.getFd();
				}
			},
			kcClick:function(e){
			...
			getFd(){//获取分段
				 let tempArray = [];
				 for (let a = 0; a < this.pageModel.list.length; a++) {
				 	let tempM = this.pageModel.list[a];
				 	// for (let b = 0; b < this.clsArray.length; b++) {
				 		let tempSe = this.fdArray[this.fdIndex];
				 		if (tempSe.value==0) {
				 			if(tempM.score>=tempSe.sectionArray[0]&&tempM.score<=tempSe.sectionArray[1]){
			...
			getList1(){//获取页面数据
				...
				this.post(this.globaData.INTERFACE_MARKINGPAPERS+url,comData,response=>{
					...
					this.hideLoading()
					let tempSection = [];
					for (let a = 0; a < response.score_section.length; a++) {
						let tempM = response.score_section[a];
						let tempSeM = {
							sectionArray:tempM,
						}
						if(a==0){
							tempSeM.text = '['+ tempM[0] +', ' +tempM[1]+']';
						}else{
							tempSeM.text = '['+ tempM[0] +', ' +tempM[1]+')';
						}
						if(tempSeM.text == this.tabBarItem.fsd_code){
							this.fdIndex = a;
						}
						tempSection.push(tempSeM);
					}
					this.fdArray = [].concat(tempSection);
		...
		onLoad(options) {
			...
			itemData.index=100
			itemData.name=itemData.newname
