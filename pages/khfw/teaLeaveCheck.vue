<template>
	<view style="background: #f2f2f2;">
		...
		<uni-card isShadow style="margin: 20px 5px 0;" @click="gotoDetail(item)" v-for="(item,index) in pagedata" :key="index">
			...
			<span class="subTea">{{item.dpt_name}} {{item.user_name}}</span>
			<uni-row style="margin-top: 25px;">
				<uni-col :span="6">
					<view class="rowStyle">请假类别：</view>
				</uni-col>
				<uni-col :span="18">
					<view class="rowStyle">{{item.type_name}}</view>
				</uni-col>
				<uni-col :span="6">
	...
	export default {
		...
		methods: {
			...
			getList0(){
				...
				// 2.6.审批列表页
				this.post(this.globaData.INTERFACE_KHFW + 'vacationApplication/checkPage', comData, (data0, data) => {
					this.hideLoading();
					if (data.code == 0) {
