<template>
	<view style="background: #f2f2f2;">
		<mynavBar ref="mynavBar" :navItem='navItem' :personInfo='personInfo' text="签到" :textClick="textClick">
		</mynavBar>
						...
						<view v-for="(item1,index1) in item0.child" :key="index1">
							<view v-for="(item2,index2) in item1.child" :key="index2">
								<uni-col :span="3" v-if="item2.setValue == 1">
									<view class="rowClass" :style="{background:item2.selectFlag == 1?'#2c96bd':item2.sign_status == 1?'':'#aaaaaa',color:item2.selectFlag == 1||item2.sign_status == 1?'white':''}"
...
<script>
	...
	export default {
		...
		methods: {
			clickSign(model){
				// console.log('clickSign0:'+JSON.stringify(this.selectModel))
				...
				// console.log('clickSign1:'+JSON.stringify(this.selectModel))
			},
			setDate() {
			...
			textClick() {
				if (this.selectModel.id) {
					...
					this.post(this.globaData.INTERFACE_KHFW + 'afterClassSignTec/sign', comData, (data0, data) => {
						this.hideLoading();
