<template>
	<view>
		...
		<view style="display: flex;height: 40px;align-items: center;padding:0 10px;justify-content: space-between;">
			<view style="font-size: 14px;color: #000000;">{{navItem.data.paper_name}}</view>
		...
		<uni-popup ref="alertDialog" type="dialog">
			<uni-popup-dialog type="warn" title="确定要退出练习吗？" content="退出后不保存本次练习记录" @confirm="dialogConfirm">
			</uni-popup-dialog>
		</uni-popup>
	</view>
</template>
			...
			changeIndex(d) {
				...
				if (des >= this.questions.length) {
					...
					let item = {
						...
						name: this.navItem.data.paper_name,
						titleName: this.navItem.text,
						usedTime: this.navRight,
						index_code: this.index_code,
						answerArray: answerArray,
						diskey: this.navItem.data.diskey,
					}
					let that = this
