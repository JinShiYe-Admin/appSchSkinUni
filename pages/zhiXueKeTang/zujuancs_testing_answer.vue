<template>
	<view>
		<mynavBar ref="mynavBar" :navItem='navItem' :personInfo='personInfo' :text="navRight" :textClick="textClick">
		</mynavBar>
		<view style="display: flex;height: 40px;align-items: center;padding:0 10px;justify-content: space-between;">
			<view style="font-size: 14px;color: #000000;">{{navItem.questionData.report.name}}</view>
		...
		<view style="display: flex;padding:0 15px;">
			<view style="display: flex;">
				...
				<text
					style="font-size: 12px;color: #00CFBD;margin-left: 3px;">{{getAnswerName(questions[index].answer)}}</text>
			</view>
			<view style="display: flex;margin-left: 10px;">
				<text style="font-size: 12px;">你选的：</text>
				<text
					style="font-size: 12px;color: #f76260;margin-left: 3px;">{{getAnswerName(questions[index].stu_answer)}}</text>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.navRight = [{
				value: '用时：' + itemData.questionData.report.used_time,
				style: {
					fontSize: 12,
					color: '#FFFFFF'
				}
