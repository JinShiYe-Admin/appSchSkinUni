<template>
	<view>
		<my-swiperPage ref='swiperPageIndex' :allValue="stList" :showBackColor="true" @swiperPagechange='changeSwiper'>
		</my-swiperPage>
		...
		<view class="mui-row" style="padding: 0 10px;">
			...
			<view>
				<h5
					...
					我的答案：</h5>
				<p v-if="stu_answer.length==0&&stu_answer_img_url.length==0" class="noData" style="margin-top: 15px;">
					暂无数据</p>
				<view v-for="(item,index) in stu_answer" :key='index'>
					<p style="font-size: 14px;color: #666;margin-left: 15px;margin: 5px 10px;">{{item}}</p>
				</view>
				<img v-if="stu_answer_img_url.length>0" style='margin-top: 5px;margin-bottom: 5px;'
					:src="stu_answer_img_url" width="100%" @click='clickImg(stu_answer_img_url)'>
			</view>
			<view>
				<h5
					style="text-align: center;padding-top: 10px;padding-bottom: 10px;background: #7FFFD4;width: 80px;color: gray;">
