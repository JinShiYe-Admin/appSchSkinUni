<template>
	<view>
		...
		<view class="mui-row" v-for="(currentModel,index) in currentList" :key='index'>
			<h5 style="text-align: center;padding-top: 10px;padding-bottom: 10px;background: #7FFFD4;color: gray;">题号：{{currentModel.question_number}}</h5>
			<view class="mui-row" style="padding: 0 10px;">
				<view style="margin-top: 15px;">
						<p style='margin-left: 10px;color: gray;font-size: 14px;'>当前题目的得分信息：</p>
						<uni-row style="margin: 10px 20px;">
							<uni-col style="height: 30px;" :span="12">
								<view style="font-size: 13px;color: gray;">分值：{{currentModel.detailModel.question_score}}分</view>
							</uni-col>
							<uni-col style="height: 30px;" :span="12">
								<view style="font-size: 13px;color: gray;">我的得分：{{currentModel.detailModel.score}}分</view>
							</uni-col>
							<uni-col style="height: 30px;" :span="12">
								<view style="font-size: 13px;color: gray;">年级满分人数：{{currentModel.detailModel.full_score_stuc}}人</view>
							</uni-col>
							<uni-col style="height: 30px;" :span="12">
								<view style="font-size: 13px;color: gray;">年级均分：{{currentModel.detailModel.grd_avg_score}}分</view>
							</uni-col>
						</uni-row>
				</view>
				<view style="margin-bottom: 10px;">
					...
					<p v-if="currentModel.content.length>0" style="font-size: 14px;color: #666;" v-html='currentModel.content'></p>
				...
				<view>
					...
					<p v-if="currentModel.stu_answer.length==0&&currentModel.stu_answer_img_url.length==0" class="noData" style="margin-top: 15px;">暂无数据</p>
					...
					<img v-if="currentModel.stu_answer_img_url.length>0" style='margin-top: 5px;margin-bottom: 5px;' :src="currentModel.stu_answer_img_url" width="100%" @click='clickImg(currentModel.stu_answer_img_url)'>
				</view>
				<view>
