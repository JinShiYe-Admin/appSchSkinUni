							...
							<text slot="body" class="slot-box slot-text" @click.stop="toDetails(item)">
								<view style="display: flex;align-items: center;width: 81vw;">
									...
									<view style="display: flex;flex-direction: column;margin-left: 8px;flex: 1;">
										<view class="detail-text-good">{{item.question_type_name}}</view>
									</view>
								</view>
							</text>
						</uni-list-item>
						...
						<template v-if="total_list.length>0">
							<view style="padding-bottom: 5px;">
								<uni-row style="background: #e7e3e3;">
									<uni-col :span="11"><p class="scoreDetail">题型名称</p></uni-col>
									<uni-col :span="5"><p class="scoreDetail">考察次数</p></uni-col>
									<uni-col :span="6"><p class="scoreDetail">平均得分率</p></uni-col>
									<uni-col :span="2"><p class="scoreDetail"></p></uni-col>
								</uni-row>
								<view @click="toDetails(item)" v-for="(item,index) in total_list" :key='index' style="margin-top: 5px;font-size: 13px;text-align: center;">
									<uni-row style="padding: 5px 0;">
										<uni-col :span="11" style="word-break: break-all;">{{item.question_type_name}}</uni-col>
										<uni-col :span="5">{{item.count}}</uni-col>
										<uni-col :span="6">{{item.radio+'%'}}</uni-col>
										<uni-col :span="2"><uni-icons type="forward" size="16" color="#bbb"></uni-icons></uni-col>
									</uni-row>
									<view v-if="index<total_list.length-1" class="line"></view>
								</view>
							</view>
						</template>
						<template v-else>
	...
	export default {
		...
		methods: {
			...
			getTurnSituation(){//获取知识点分析数据
				 ...
				 this.post(this.globaData.INTERFACE_MARKINGPAPERS+'stuAnalysis/questionTypeAnalysis',params,response=>{
				 	 console.log("response: " + JSON.stringify(response));
					 this.good_list=response.good_list
					 this.bad_list=response.bad_list
					 this.total_list=response.list
				 })
			},
			toDetails(item){
				...
				if(item.paper_question_number.split(',').length>0){
					...
					util.openwithData('/pages/homeworkAndWeekTest_stu/qusetionAnalysisDetail',item,{})
				}else{
