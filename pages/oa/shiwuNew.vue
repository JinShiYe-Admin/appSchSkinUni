		...
		methods: {
			...
			checkSmsCont() {
				...
				if (this.smsConfig.content_type == 't') {
					...
					tempContent = this.content.substr(0, 350 - signCount);
				} else if (this.smsConfig.content_type == 'tc') {
					tempContent = '【' + this.title + '】' + this.content.substr(0, 350 - signCount - this.title.length - 2);
				}
				if (this.addSign && this.smsSend) {
					tempContent += '[' + this.personInfo.user_name + ']'
				}
			...
			getSmsConfig() { //获取短信配置
				let comData = {
					msg_type: this.OA_MSG_SMS.AFFAIR.MSG_TYPE,
				...
				this.post(this.globaData.INTERFACE_HR_SUB + 'smsConf/getConf', comData, response => {
					...
					if (response) {
						...
						let local_types = this.OA_MSG_SMS.AFFAIR.USER_TYPE.split(",");
						let send = false;
			...
			submitData(encNameStr, encAddrStr) {
				...
				//28.回复通知公告
				this.post(this.globaData.INTERFACE_OA + 'cooperateNotice/addNotice', tempData, (data0, data) => {
					// this.canSub = true;
					this.hideLoading();
					if (data.code == 0) {
						if (this.smsSend) {
							...
							for (var i = 0; i < selectData.length; i++) {
								let obj = {
									gen_type: this.OA_MSG_SMS.AFFAIR.USER_TYPE,
							...
							this.post(this.globaData.INTERFACE_HR_SUB + 'smsRecord/save', comData, (data0,
								...
								if (datas.code == 0) {
									...
									var dosetData = {
										...
										msgType: this.OA_MSG_SMS.AFFAIR.MSG_TYPE, //信息类型
									...
									this.post(this.globaData.INTERFACE_OA +
										'cooperateNotice/doSetSms4Notice',
										dosetData, (data0, doData) => {
											this.hideLoading();
											const eventChannel = this.getOpenerEventChannel()
											eventChannel.emit('refreshShiWuIndex');
											uni.navigateBack();
										});
								} else {
									this.hideLoading();
				...
				util.openwithData("/pages/oa/selectPeople1", data, {
					refreshSetPeople(data) { //子页面调用父页面需要的方法
						...
						selectPeople.map(user => {
							if (user.user_code == _this.personInfo.user_code) {
								incloudMe = true;
							}
						});
						if (!incloudMe) {
							let obj = {}
							obj.user_code = _this.personInfo.user_code;
							obj.user_name = _this.personInfo.user_name;
							obj.user_img = _this.personInfo.img_url ? _this.personInfo.img_url : '';
