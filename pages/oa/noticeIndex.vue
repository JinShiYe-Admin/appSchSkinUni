		...
		methods: {
			...
			getPageList() {
				...
				if (this.semFlag == 3) {
					...
					url = this.globaData.INTERFACE_OA + 'notice/getSendNotice';
				} else {
					comData.receiveManId = this.personInfo.user_code; //接收人ID
					url = this.globaData.INTERFACE_OA + 'notice/getReceiveNotice';
				}
				if (this.semFlag == 0) {
					comData.status = '0'; //状态,0 全部1 已读2 未读
					comData.page_number = this.semFlag0Data.pageIndex; //当前页数
				} else if (this.semFlag == 1) {
					comData.status = '2'; //状态,0 全部1 已读2 未读
					comData.page_number = this.semFlag1Data.pageIndex; //当前页数
				} else if (this.semFlag == 2) {
					comData.status = '1'; //状态,0 全部1 已读2 未读
