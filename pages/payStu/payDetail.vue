<template>
	<view>
		...
		<uni-card isShadow>
			...
			<view style="font-size: 14px;margin-top: 10px;text-align: center;">{{personInfo.grd_name}} {{personInfo.cls_name}} {{personInfo.stu_name}}</view>
		</uni-card>
		<uni-card isShadow>
			...
			<view style="font-size: 36px;color: #333333;margin-top: 5px;text-align: center;">
				{{pageData.pay_price}}</view>
			<view v-if="pageData.pay_price==0" class="noMoney">免缴</view>
		</uni-card>
		<view v-if="navItem.status == 1&&navItem.pay_status == 0 &&pageData.pay_price>0" class="wxPay" @click="wxPay()">
			微信支付
		</view>
