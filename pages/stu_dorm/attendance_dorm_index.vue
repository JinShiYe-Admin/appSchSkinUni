	...
	<view>
		...
		<view style="padding-top:44px;">
			<uni-list :border="false">
				<uni-list-item showArrow clickable @click="toDetails(item)" :key="index"
					v-for="(item,index) in pagedata" :border="true">
						...
						<uni-row>
							...
							<uni-col :span="24">
								<view class="detail-text">
									楼房:{{item.dorm_name}}&ensp;楼层:{{item.floor_num}}层&ensp;房间:{{item.room_name}}&ensp;床位:{{item.bed_num}}
								</view>
							</uni-col>
							<uni-col :span="12">
								<view class="detail-text">考勤:{{item.item_name}}</view>
							</uni-col>
							<uni-col :span="12">
								<view class="detail-text" style="text-align: right;">{{item.attendance_date}}</view>
							</uni-col>
						</uni-row>
					</view>
				</uni-list-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			addClick() {
				let that = this
				util.openwithData('/pages/stu_dorm/attendance_dorm_add', {
