<template>
	<view>
		...
		<view style="padding:5px 10px 0;margin-top: 5px;">
			<uni-row v-for="(item,index) in selectGradeClassesNames" :key="index">
				<uni-col :span="24">
					<view style="font-size: 12px;color: #787878;margin: 5px 0;">{{item.grdcls_names}} :</view>
				</uni-col>
				<uni-col :span="24">
					<view style="font-size: 12px;color: #787878;margin: 5px 0 0 10px;">{{item.stu_names}}</view>
				</uni-col>
			</uni-row>
		</view>
		<!-- #ifdef H5 -->
...
<script>
	...
	export default {
		...
		methods: {
				...
				util.openwithData('/pages/schhome/selectGrdClsStu',data,{
					refreshSetPeople(data){//子页面调用父页面需要的方法
						...
						data.data.map(grd_item=>{
							 grd_item.clsList.map(cls_item=>{
								 let stu_names=[]
								 cls_item.stuList.map(stu_item=>{
									 stu_names.push(stu_item.stu_name)
								 })
								 let obj={
								 	grdcls_names:grd_item.name+' '+cls_item.name,
								 	stu_names:stu_names.join('，')
								 }
								selectGradeClassesNames.push(obj)
							})
						})
						that.selectGradeClassesNames=selectGradeClassesNames
					}
				})
			},
			valueChange(e){
			...
			getSmsConfig(){//获取短信配置
				let comData={
					msg_type: this.MSG_SMS.PERFORMANCE.MSG_TYPE,
				...
				this.post(this.globaData.INTERFACE_HR_SUB+'smsConf/getConf',comData,response=>{
					...
					if (response && response.user_types) {
						...
						let local_types=this.MSG_SMS.PERFORMANCE.USER_TYPE.split(",");
						let send=false;
			...
			submitData(){
				...
				for (var i = 0; i < selectData.length; i++) {
					...
					for (var z = 0; z < cls.length; z++) {
						...
						let obj={
							...
							grd_code:grdCode,
							grd_name:grdName,
							cls_code:cls[z].value,
							cls_name:cls[z].name,
						}
						let user_codes=[],user_names=[]
						for (var x = 0; x < stus.length; x++) {
							let stu=stus[x]
							user_codes.push(stus[x].stu_code)
							user_names.push(stus[x].stu_name)
						}
				...
				let comData={
					...
					tousers:touser,
					hr_smsid:0,//固定值
					sms_msgtype_code:this.MSG_SMS.PERFORMANCE.SMS_TYPE,
					...
					for (var z = 0; z < cls.length; z++) {
						...
						for (var x = 0; x < stus.length; x++) {
							let obj={
								...
								cls_code:cls[z].value,
								cls_name:cls[z].name,
								stu_code:stus[x].stu_code,
								stu_name:stus[x].stu_name,
								gen_code:'',
								gen_name:'',
							}
							touser.push(obj)
