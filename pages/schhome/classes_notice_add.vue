<template>
	<view>
		...
		<view class="uni-flex uni-row form-view"  style="padding:5px 10px 0;">
			<view style="font-size: 13px;color: #787878;">{{selectGradeClassesNames.join('，')}}</view>
		</view>
		<!-- #ifdef H5 -->
		...
		<!-- #endif -->
			<view class="popup-content">
				<view class="text" @click="checkNow">
				 <text>即时发送</text>
				 <uni-icons v-show="nowIcon" type="circle-filled" color="#00CFBD" size="14"></uni-icons>
				</view>
				<view class="line2"></view>
				<view class="text"  @click="checkDelay">
					<text>延时发送</text>
					<picker v-show="delayIcon" mode="multiSelector" ref="picker" :value="multiIndex" :range="multiArray" @change="valueChange">
					...
					<uni-icons v-show="delayIcon" type="circle-filled" color="#00CFBD" size="14"></uni-icons>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
					...
					refreshSetPeople(data){//子页面调用父页面需要的方法
						...
						data.data.map(grd_item=>{
							 grd_item.clsList.map(cls_item=>{
								selectGradeClassesNames.push(grd_item.name+cls_item.name)
							})
						})
			...
			getSmsConfig(){//获取短信配置
				let comData={
					msg_type: this.MSG_SMS.CLASS.MSG_TYPE,
				...
				this.post(this.globaData.INTERFACE_HR_SUB+'smsConf/getConf',comData,response=>{
					...
					if (response && response.user_types) {
						...
						let local_types=this.MSG_SMS.CLASS.USER_TYPE.split(",");
						let send=false;
			...
			submitData(){
				...
				let comData={
					...
					tousers:touser,
					hr_smsid:0,//固定值
					sms_msgtype_code:this.MSG_SMS.CLASS.SMS_TYPE,
			...
			sendSMS(id,delayTime,comment){//发送短信
				...
				for (var i = 0; i < selectData.length; i++) {
					...
					for (var z = 0; z < cls.length; z++) {
						let obj={
							gen_type:this.MSG_SMS.CLASS.USER_TYPE,
							dpt_code:'',
							dpt_name:'',
							grd_code:grdCode,
							grd_name:grdName,
