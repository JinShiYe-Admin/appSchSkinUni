		...
		methods: {
			...
			getList0(){//获取页面数据
				let comData={
					...
					begin_time: this.moment(this.time).format("YYYYMMDD"), //查询开始时间
					end_time: this.moment(this.time).format("YYYYMMDD"), //查询结束时间
					op_code: 'index',
					index_code: this.index_code,
				}
				this.post(this.globaData.INTERFACE_ATTENDAND+'attendDptQry/getDptDayAttends',comData,response=>{
				    console.log("responseaaa: " + JSON.stringify(response));
					this.hideLoading()
			...
			toDetails(item){
				...
				item.time=this.time
				util.openwithData('/pages/teachercAttendance/attendanceDetail',item,{})
			}
		},
		onLoad(options) {
