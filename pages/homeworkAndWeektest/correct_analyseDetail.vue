<template>
	<view>
		...
		<view  style="text-align: center;margin-top: 10px;">
			<text class="title-text">{{itemData.knowledge_name?itemData.knowledge_name:itemData.question_type_name}} 相关试题</text>
		</view>
		...
		<view class="content" style="margin-top: 10px;padding-bottom:35px ;">
			...
			<uni-card isShadow :isFull="true">
				<text class="content-box-text">
					...
					<uni-row v-for="(model,index) in score_section_list" :key="index" style="margin: 5px 0 0 0;text-align: center;">
						<uni-col style="" :span="8">
					...
					<view v-if="score_section_list.length==0" style="text-align: center;font-size: 14px;margin-top: 5px;">暂无</view>
				</text>
			</uni-card>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			setTimeout(function() {
				_this.score={series:[{name:"平均得分率",data:(itemData.radio*1/100).toFixed(2),color:"#499df8"}]}
				_this.scoreOpts={title:{name:itemData.radio+'%',fontSize:24,color:'#499df8'},subtitle:{name:'平均得分率',color:'#666666',fontSize:13}}
			}, 500);
