<template>
	<view>
		...
		<view class="tabs-fixed">
			...
			<view style="font-size: 12px;text-align: center;color: gray;">{{yearList[yearIndex].year_name}}
				{{termList[termIndex].term_name}}
			</view>
					...
					<uni-card isShadow :isFull="true" style="margin-top: 0px;">
						<text class="content-box-text">
							<uni-row style="margin: 0px -8px 10px -8px;">
								...
								<uni-col style="" :span="8">
									<p class="stuScoreDetail">已交次数：{{semFlag0Data.count}}</p>
								</uni-col>
								<uni-col style="" :span="8">
									<p class="stuScoreDetail">未交次数：{{semFlag0Data.miss_count}}</p>
								</uni-col>
							</uni-row>
							<uni-row style="background-color: #e5e3e3;height: 40px;margin: 0px -12px 0 -12px;">
								<uni-col style="" :span="14">
									<p class="scoreDetail">作业名称</p>
								</uni-col>
								<uni-col style="" :span="4">
									<p class="scoreDetail">状态</p>
								</uni-col>
								<uni-col style="" :span="4">
									<p class="scoreDetail">成绩</p>
								</uni-col>
							...
							<uni-row v-for="(model,index) in semFlag0Data.dataList" :key="index" style="margin: 0px -8px 0 -8px;">
								<view @click="clickLi(model)">
									<uni-col style="" :span="14">
										<p class="stuScoreDetail">{{model.name}}</p>
										<view class="card-line" style="margin-left: 10px;"></view>
									</uni-col>
									<uni-col style="" :span="4">
										<p class="stuScoreDetail">{{model.state}}</p>
									</uni-col>
									<uni-col style="" :span="4">
										<p class="stuScoreDetail">{{model.score}}</p>
									</uni-col>
									<uni-col style="margin-top: 5px;" :span="2">
										<uni-icons v-if="model.state=='已交'" style="font-size: 14px;margin-left: 10px;"
											type="arrowright" size="20"></uni-icons>
									</uni-col>
								</view>
							</uni-row>
		...
		<uni-popup ref="popupSelect" type="center" style="background-color: white;">
			<scroll-view
				...
				<picker style="margin-top: 10px;" @change="clickYear" :value="yearIndexTemp" :range="yearList" range-key='year_name'>
					<view class="uni-input pickBorder">{{yearList[yearIndexTemp].year_name}}</view>
				</picker>
				<picker style="margin-top: 10px;" @change="clickTerm" :value="termIndexTemp" :range="termList" range-key='term_name'>
					<view class="uni-input pickBorder">{{termList[termIndexTemp].term_name}}</view>
				</picker>
				<picker style="margin-top: 10px;" @change="clickGrd" :value="grdIndexTemp" :range="grdArray" range-key='name'>
					<view class="uni-input pickBorder">{{grdArray[grdIndexTemp].name}}</view>
				</picker>
				<picker style="margin-top: 10px;" @change="clickCls" :value="clsIndexTemp" :range="clsArrayTemp" range-key='name'>
					<view class="uni-input pickBorder">{{clsArrayTemp[clsIndexTemp].name}}</view>
				</picker>
				<picker style="margin-top: 10px;" @change="clickSub" :value="subIndexTemp" :range="subArrayTemp" range-key='name'>
					<view class="uni-input pickBorder">{{subArrayTemp[subIndexTemp].name}}</view>
				</picker>
				<view class="pupName">学生：</view>
				<uni-row>
					<uni-col :span="12" v-for="(model,index) in stuArrayTemp" :key="index">
						<view :class="index == stuIndexTemp?'popSelect':'popNoSelect'" @click="clickStu(index)">
							{{model.name}}
						</view>
					</uni-col>
				</uni-row>
...
<script>
	...
	export default {
		...
		methods: {
			...
			clickYear(e) {
				this.yearIndexTemp = e.target.value;
			},
			clickTerm(e) {
				this.termIndexTemp = e.target.value;
			},
			clickGrd(e) {
				this.grdIndexTemp = e.target.value;
				this.getCls(0);
			},
			clickCls(e) {
				this.clsIndexTemp = e.target.value;
				this.getSub(0);
			},
			clickSub(e) {
				this.subIndexTemp = e.target.value;
			},
			clickStu(index){
				this.stuIndexTemp = index;
			},
			popSure(flag) {
				if (flag == 0) {
					...
					this.stuIndexTemp = this.stuIndex;
					this.clsArrayTemp = [].concat(this.clsArray);
					this.stuArrayTemp = [].concat(this.stuArray);
					...
					this.stuIndex = this.stuIndexTemp;
					this.subIndex = this.subIndexTemp;
					this.clsArray = [].concat(this.clsArrayTemp);
					this.stuArray = [].concat(this.stuArrayTemp);
			...
			getSub(flag) { //获取科目、学生
				...
				this.post(this.globaData.INTERFACE_HR_SUB + 'acl/dataRange', comData, response => {
					...
					if (response.sub_list.length > 0) {
						...
						this.subIndexTemp = 0;
						this.stuArrayTemp = response.stu_list;
						this.stuIndexTemp = 0;
						if (flag == 1) {
							this.subArray = response.sub_list;
							this.stuArray = response.stu_list;
							this.getStuAnalysisList();
						}
					} else {
						this.subArray = [];
			...
			getStuAnalysisList() {
				var comData = {
					...
					stu_name: this.stuArray[this.stuIndex].name,
				...
				this.showLoading();
				// 1.211.学生上交作业总结
				this.post(this.globaData.INTERFACE_MARKINGPAPERS + 'stuAnalysis/turnSituation', comData, (data0, data) => {
					this.hideLoading();
					if (data.code == 0) {
						this.semFlag0Data.miss_count = data.data.miss_count;
						this.semFlag0Data.count = data.data.count;
					} else {
						this.showToast(data.msg);
					}
				});
				//1.212.学生统计分析分页
				this.post(this.globaData.INTERFACE_MARKINGPAPERS + 'stuAnalysis/page', comData, (data0, data) => {
					this.hideLoading();
					if (data.code == 0) {
