<template style="background-color: red;">
	<view class="warp">
		...
		<uni-card style="font-weight: 900;" v-for="(model, index0) in tabarMore" :index="index0" :key="index0">
			<view style="text-align: center;font-size: 18px;font-weight: 700;">{{model.text}}</view>
			<view  v-for="(item, index) in model.childList" :index="index" :key="index">
				<view style="margin-top: 10px;">
					<span style="width: 15px;height: 15px;background: #00CFBD;border-radius: 50%;display: block;"></span>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			_this = this;
			// 获取未读推送消息数的监听
			uni.$on('setPushCountMore', function(data) {
				console.log('setPushCountMore000000000000000000000')
