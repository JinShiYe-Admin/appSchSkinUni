	...
	<view>
		...
		<view v-if="detailData.id" style="margin-top: 5px;">
				...
				<view v-for="(extraFile,index) in detailData.enc_list" :key='index'>
					<view class="encName">附件:
						<a class="" style="font-size: 13px;color: #3c9bfe;margin-left: 10px;"
							@click="checkEnc(extraFile.enc_addr)">附件{{index+1}}</a>
					</view>
				</view>
			</template>
			...
			<view class="approvePerCls" style="margin: 0 15px;" v-if="detailData.copy_mansArray.length>0">
				{{detailData.copy_mansArray.join(',')}}</view>
	...
	export default {
		...
		onLoad(options) {
			...
			uni.$on('clickLeft', (data) => {
				if (_this.isDel == 1) {
					...
					eventChannel.emit('refreshApproveIndexList', {});
				}
			})
		},
		onShow() {
		...
		methods: {
			...
			getDetail() {
				...
				this.showLoading();
				//16.通过ID获取费用审批
				this.post(this.globaData.INTERFACE_COSTMS + 'costApply/getCostApproveById', comData, (data0, data) => {
					this.hideLoading();
					if (data.code == 0) {
